...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
        ...
        <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-slate-500">
                    Filter data pendaftar berdasarkan periode, rentang tanggal, status, atau asal sekolah.
                </p>
            </div>
            ...
            <div class="flex flex-wrap gap-2">
                ...
                {{-- EXPORT PDF --}}
                <a href="{{ route('admin.reports.ppdb.pdf', request()->query()) }}"
                   target="_blank"
                ...
                {{-- EXPORT EXCEL --}}
                <a href="{{ route('admin.reports.ppdb.excel', request()->query()) }}"
        ...
        {{-- FILTER --}}
        <form method="GET"
              action="{{ route('admin.reports.ppdb.index') }}"
              class="mt-5 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-6 gap-3">
            ...
            <div>
                ...
                >
                    ...
                    <option value="documents_valid"
                        {{ request('status') == 'documents_valid' ? 'selected' : '' }}>
                        Berkas Valid
                    </option>

                    <option value="documents_invalid"
                        {{ request('status') == 'documents_invalid' ? 'selected' : '' }}>
                        Berkas Ditolak
                    </option>

                    <option value="graded"
                        {{ request('status') == 'graded' ? 'selected' : '' }}>
                        Nilai Diproses
                    </option>
            ...
            <div class="flex items-end gap-2">
                ...
                <a
                    href="{{ route('admin.reports.ppdb.index') }}"
                    class="w-full rounded-lg border border-slate-300 bg-white px-4 py-2 text-center text-sm font-medium text-slate-600 hover:bg-slate-50 transition"
                >
                    Reset
                </a>
            ...
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                ...
                <tbody class="divide-y divide-slate-200">
                    ...
                    @forelse($registrations as $index => $registration)
                        ...
                        @php
                            $selectedMajor = optional($registration->majorChoices->get(0))->major?->name ?? '-';
