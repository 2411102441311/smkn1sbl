...
@section('content')
    ...
    <section class="max-w-3xl mx-auto px-6 py-10">
        ...
        <p class="text-sm text-slate-500 mb-6">
            Langkah 6 dari 7 — dihitung otomatis dari nilai rapor.
        </p>
        ...
        @if($recommendedMajor)
            <div class="bg-gradient-to-br {{ $recommendedMajor->color_from }} {{ $recommendedMajor->color_to }} rounded-3xl p-8 text-center text-white mb-6">
                ...
                <div class="mx-auto mb-4">
                    @include('partials.major-badge', [
                        'major' => [
                            'name' => $recommendedMajor->name,
                            'logo' => $recommendedMajor->logo,
                            'icon' => $recommendedMajor->icon,
                            'color_from' => $recommendedMajor->color_from,
                            'color_to' => $recommendedMajor->color_to,
                        ],
                        'size' => 'lg'
                    ])
                </div>

                <h2 class="font-display font-extrabold text-2xl">
                    {{ $recommendedMajor->name }}
                </h2>

                <p class="text-white/90 text-sm mt-2 max-w-md mx-auto">
                    {{ $recommendedMajor->description }}
                </p>
        ...
        <div class="bg-white rounded-2xl border border-skblue-100 p-6 mb-8">
            ...
            <p class="text-xs font-bold uppercase tracking-wide text-skblue-500 mb-4">
            ...
            <div class="space-y-3">
                ...
                @foreach($scores as $slug => $score)
                    ...
                    @php
                        $major = $majors->firstWhere('slug', $slug);
                    @endphp
                    ...
                    <div>
                        ...
                        <div class="flex justify-between text-sm mb-1">
                            ...
                            <span class="font-medium text-slate-700">
                                {{ $major->code ?? strtoupper($slug) }}
                            </span>

                            <span class="text-skblue-600 font-semibold">
                                {{ round($score * 100, 1) }}%
                            </span>
                        ...
                        <div class="w-full h-2 bg-skblue-50 rounded-full overflow-hidden">
                            ...
                            <div
                                class="h-full bg-skblue-500 rounded-full"
        ...
        <a
            href="{{ route('ppdb.wizard.majorChoice') }}"
